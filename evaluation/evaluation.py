...
def HiCRep(mat1, mat2):
    ...
    for index in range(num_of_samples):
        ...
        r1 = [np.mean(mat1_K[k]*mat2_K[k])-np.mean(mat1_K[k])*np.mean(mat2_K[k]) for k in range(len(K_index))]
        r2 = [np.sqrt(np.var(mat1_K[k]) * np.var(mat2_K[k])) for k in range(len(K_index))]
...
if __name__ == '__main__':
    ...
    genome_disco = GenomeDISCO(highres_sub, lowres_sub, t = 5)
