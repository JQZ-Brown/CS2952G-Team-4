...
def train(lowres,highres, outModel):
    ...
    with open('HindIII_train.txt', 'w') as log:
        for epoch in range(0, 3500):
            ...
            print('-------', i, epoch, running_loss/i, strftime("%Y-%m-%d %H:%M:%S", gmtime()),flush=True)
	...
	# save     the model every 100 epoches
            if (epoch % 100 == 0):
                torch.save(Net.state_dict(), '/users/tdefrosc/data/tdefrosc/coco_herbarium/' + outModel + str(epoch) + str('.model'))
        pass
